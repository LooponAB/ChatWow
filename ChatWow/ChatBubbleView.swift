...
class ChatBubbleView: UIView
{
	...
	override func draw(_ rect: CGRect)
	{
		...
		if legOnLeft
		{
			...
			bezier2Path.move(to: CGPoint(x: 8, y: 0))
			...
			bezier2Path.addCurve(to: CGPoint(x: 8, y: 0), controlPoint1: CGPoint(x: 20.74, y: 19.99), controlPoint2: CGPoint(x: 8, y: 0))
			bezier2Path.close()
			bubbleColor.setFill()
			bezier2Path.fill()
		...
		else
		{
			...
			bezierPath.move(to: CGPoint(x: 8.94, y: 0))
			...
			bezierPath.addCurve(to: CGPoint(x: 8.94, y: 0), controlPoint1: CGPoint(x: -3.8, y: 19.99), controlPoint2: CGPoint(x: 8.94, y: 0))
			bezierPath.close()
			bubbleColor.setFill()
			bezierPath.fill()
